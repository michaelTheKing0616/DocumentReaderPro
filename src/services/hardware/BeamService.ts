...
class BeamService {
  ...
  async connect(): Promise<boolean> {
    try {
      this.config = { type: 'webgazer', connected: true };
      this.isConnected = true;
      this.startGazeStream();
      return true;
    } catch (error) {
      logger.error('Beam connection error', {
  ...
  private startGazeStream() {
    ...
    this.interval = startHardwareGazeStream('Beam', this.gazeCallback, 30, () => this.isConnected);
  }
...
export default new BeamService();
